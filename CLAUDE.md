...
公開バンドルは **gzip 25KB 以内**を上限とする（現在 24.8KB）。
...
- ボカシは Canvas の `filter: blur()`。画像の外側を透明として扱う仕様のため、画像の縁に
  ...
  マスクとして破綻する。
- `ctx.filter` の有無は文字列型では判定できない（WebKit は代入できるのに効かない）。
  実際にぼかして滲みを測る `supportsCanvasBlur()` で判定し、効かない環境では
  縮小 + 移動平均（box blur）へフォールバックする。**このフォールバックの縮小率を
  ぼかし半径に連動させてはいけない**。連動させると、強さの変化が段階的になり、
  さらに表示倍率を変えるたびにぼかしの粗さが変わる（実際に不具合を出した）。
