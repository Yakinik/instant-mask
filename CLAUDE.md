...
- レイヤ座標は画像ピクセル基準で保持する。`renderScene()` は常に画像座標で描き、
  表示倍率と DPR は**呼び出し側が transform で与える**（クリアも呼び出し側の責任）。
  表示と書き出しで同じ関数を通るので、見た目と保存結果が一致する。この構造を崩さない。
- 表示用キャンバスは**画面サイズ固定**にして、見えている範囲だけを描く。画像全体ぶんを
  持つと、拡大時に画素数が膨らんでメモリと描画時間を食ううえ、上限に当たると
  逆に解像度が足りなくなる。画面固定なら倍率に依らず常に等倍で描ける。
- ぼかしの作業バッファなど、内部で解像度を決める処理は `ctx.getTransform()` から
  実際の描画スケールを取ること。画像座標だけで決めると拡大時にそこだけ粗くなる。
- 絵文字は Apple Color Emoji などのビットマップフォントなので、大きく拡大すると
  ドットが見える。これはフォント側の制約で、解像度を上げても解決しない
  （テキストはベクターなので拡大しても鮮明）。
